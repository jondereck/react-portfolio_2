...
const providerKeyAliasesByUiKey = {
  generationMode: ['generationMode', 'generation-mode'],
  bodyType: ['bodyType', 'body-type'],
  breastsSize: ['breastsSize', 'breasts-size'],
  assSize: ['assSize', 'ass-size'],
};
...
export function getUnclothyGenerationModeLabel(settings) {
  if (!settings || typeof settings !== 'object') return null;
  const value = settings.generationMode ?? settings['generation-mode'];
  if (value == null) return null;
  const normalized = String(value).trim();
  return normalized || null;
}

export function expandUnclothyProviderSettingsAliases(settings) {
  const source = settings && typeof settings === 'object' ? settings : {};
  const expanded = { ...source };

  for (const [uiKey, aliases] of Object.entries(providerKeyAliasesByUiKey)) {
    const value = aliases.map((key) => source[key]).find((candidate) => candidate != null && String(candidate).trim());
    if (value == null) {
      continue;
    }
    ...
    for (const alias of aliases) {
      expanded[alias] = normalizedValue;
    }
  }
...
export function buildUnclothyProviderSettingsPayload(settings, settingsEnums) {
  ...
  return expandUnclothyProviderSettingsAliases(payload);
}
...
export function sanitizeUnclothyProviderSettings(rawSettings, settingsEnums) {
  ...
  return {
    settings: expandUnclothyProviderSettingsAliases(sanitizedSettings),
    fieldErrors,
  };
}
