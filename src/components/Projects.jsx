...
const Projects = ({ profileSlug = null }) => {
  ...
  const featuredProjects = projectList.filter((project) => Boolean(project?.isFeatured));
  const regularProjects = projectList.filter((project) => !project?.isFeatured);
  const totalPages = Math.ceil(regularProjects.length / itemsPerPage);
  const paginatedData = regularProjects.slice((page - 1) * itemsPerPage, page * itemsPerPage);
  ...
  return (
    <SectionContainer
      name="portfolio"
      title="Portfolio"
      subtitle="Selected projects that combine design quality, performance, and product thinking."
    >
      {error && (
      ...
      {loading && <p className="mb-4 text-sm text-slate-500">Loading projectsâ€¦</p>}
      {!loading && !error && projectList.length === 0 ? <p className="mb-4 text-sm text-slate-500">No projects yet.</p> : null}

      {!loading && !error && featuredProjects.length > 0 ? (
        <div className="mb-10">
          <div className="mb-4 flex items-end justify-between gap-3">
            <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Featured</h3>
            <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800" aria-hidden />
          </div>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {featuredProjects.map((project, index) => (
              <ProjectCard key={project.id ?? project.slug ?? `${project.title}-${index}`} project={project} index={index} />
            ))}
          </div>
        </div>
      ) : null}
      ...
      {!loading && !error && regularProjects.length > 0 ? (
        <div>
          {featuredProjects.length > 0 ? (
            <div className="mb-4 flex items-end justify-between gap-3">
              <h3 className="text-lg font-semibold text-slate-900 dark:text-slate-100">All Projects</h3>
              <div className="h-px flex-1 bg-slate-200 dark:bg-slate-800" aria-hidden />
            </div>
          ) : null}
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
